...
for i, example in enumerate(audio_data):
    try:
        ...
        if 'bonafide' in file_path:
            label = 'bonafide'
        elif 'spoofed_tacotron'in file_path:
            label = 'Spoofed'
        elif 'spoofed_tts'in file_path:
            label = 'Spoofed'
        else:
...
joblib.dump(scaler, "scaler2.joblib")  # Save scaler
...
joblib.dump(lr_model, "logistic_model2.joblib")
